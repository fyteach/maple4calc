...
Here `:=` means "defined/assigned to be" and `->` means "plug in". The symbol `->` is known as a functional operator in Maple. If an expression is already defined, you can also create a functional operator by using the `unapply(expression, variable)` command.
...
The function name is $f$, the independent variable is $x$ and the function rule is $\dfrac{\sqrt[3]{x}}{\abs{x+1}}$. So the function can be defined in Maple by the following command.
...
A function is defined by $y=\frac{\cos x}{2+\sin x}$. Find the $y$-value when $x=0.002$.

```

```{solution}

We first define an expression and then use the `unapply` command to evaluate the function.
...
A function is defined by $y=\frac{\sqrt{x^2-2x+2}}{2-\cos x}$. Find the $y$-value when $x=\pi$.
...
<!-- ![Maple functions help page screen shot](figs/Initial-Functions.PNG) -->
...
Another initially known function that we will use is the piecewise function.
